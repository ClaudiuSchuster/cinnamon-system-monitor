#!/usr/bin/env python3
"""Render a deterministic Adaptive System Monitor social-preview variant."""
...
def main() -> int:
    ...
    icon = repo_dir / "icon.svg"
    if not background.is_file() or not icon.is_file():
        raise SystemExit("Social-preview background or monitor icon is missing.")
    ...
    with tempfile.TemporaryDirectory(prefix="system-monitor-social-preview-") as temporary:
        ...
        subprocess.run(
            command + [str(icon_source), "-filter", "Lanczos", "-resize", "82x82", str(app_icon)],
            check=True,
        )
        subprocess.run(
            command
